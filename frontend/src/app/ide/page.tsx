...
import { getAutocompleteSuggestions, getWordAtCursor, type AutocompleteItem } from "@/lib/aoiAutocomplete"
...
export default function Home() {
  ...
  // Handle autocomplete selection using useCallback
  const selectAutocompleteItem = useCallback((item: AutocompleteItem) => {
    if (!editorRef.current) return

    const textarea = editorRef.current
    const cursorPosition = textarea.selectionStart
    const { start } = getWordAtCursor(code, cursorPosition)

    // Replace the word with the selected item's insert text
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault()
        setAutocompleteSelectedIndex((prev) => (prev - 1 + autocompleteSuggestions.length) % autocompleteSuggestions.length)
      } else if (e.key === "Enter" || e.key === "Tab") {
        if (autocompleteSuggestions.length > 0) {
          e.preventDefault()
          selectAutocompleteItem(autocompleteSuggestions[autocompleteSelectedIndex])
        }
  ...
  }, [showAutocomplete, autocompleteSuggestions, autocompleteSelectedIndex, selectAutocompleteItem])
  ...
  const calculateAutocompletePosition = () => {
    ...
    const top = rect.top + (lines.length - 1) * lineHeight + lineHeight + 8
    const left = rect.left + spanRect.width + 60 // 60 for line numbers
  ...
  const handleCodeChange = (newCode: string) => {
    ...
    const suggestions = getAutocompleteSuggestions(newCode, cursorPosition)
    ...
    >
      ...
      <div className="w-full max-w-6xl relative z-10">
          ...
          >
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium flex items-center gap-2 group">
                <div
                  className={`flex items-center justify-center h-6 w-6 rounded-full bg-gradient-to-r from-emerald-500 to-green-500 shadow-lg ${
                    theme === "dark" ? "shadow-emerald-900/30" : "shadow-emerald-500/20"
                  }`}
                >
                ...
                <span
                  className={`bg-clip-text text-transparent ${
                    theme === "dark"
                      ? "bg-gradient-to-r from-emerald-200 to-emerald-100 group-hover:from-emerald-100 group-hover:to-white"
                      : "bg-gradient-to-r from-emerald-700 to-emerald-600 group-hover:from-emerald-800 group-hover:to-emerald-700"
            ...
            >
              ...
              {/* Code editor container with syntax highlighting */}
              <div className="relative flex-1">
                {/* Syntax highlighted code (background) */}
                {code && (
                  <div
                    className="absolute inset-0 pt-10 px-4 pb-4 pointer-events-none overflow-hidden whitespace-pre-wrap break-words font-mono text-sm"
                    dangerouslySetInnerHTML={{ __html: highlightCode(code, theme) }}
                    style={{ lineHeight: "1.25rem" }}
                  />
                )}

                {/* Textarea (foreground - transparent text when code exists) */}
                <textarea
                  ref={editorRef}
                  className={`relative w-full h-full pt-10 px-4 pb-4 bg-transparent ${
                    code ? "text-transparent" : theme === "dark" ? "text-indigo-100" : "text-indigo-950"
                  } caret-${theme === "dark" ? "indigo-400" : "indigo-600"} focus:outline-none resize-none min-h-[400px] transition-colors duration-500 placeholder:${
              ...
              <div
                className={`absolute -inset-0.5 bg-gradient-to-r from-emerald-500 to-green-600 rounded-xl opacity-0 group-focus-within:${
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className={`px-6 py-2.5 rounded-lg text-white text-sm font-medium flex items-center gap-2 transition-all duration-300 ${
                  loading || !code.trim()
                    ? theme === "dark"
                      ? "bg-emerald-800/30 text-emerald-300/50 cursor-not-allowed"
                      : "bg-emerald-300 text-white cursor-not-allowed"
                    : theme === "dark"
                      ? "bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 shadow-lg hover:shadow-xl shadow-emerald-900/30 hover:shadow-emerald-900/40 hover:-translate-y-0.5"
                      : "bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 shadow-md hover:shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 hover:-translate-y-0.5"
                }`}
              >
                {loading ? (
                  <>
        ...
        >
          {/* Background glow */}
          <div
            className={`absolute -inset-1 bg-gradient-to-r from-emerald-500/10 via-green-500/10 to-teal-500/10 blur-xl ${
